...
FIGURE_NOTE_TEMPLATES: dict[str, FigureNoteTemplate] = {
    ...
    "waterbridge_count_combined": FigureNoteTemplate(
        name="跨重复水桥数量汇总时间序列",
        explanation="该图展示多个重复中桥连水分子数量的三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估水桥网络的重复性。",
        caption_example="多重复桥连水分子数量汇总图。三条重复轨迹若走势接近，且均值曲线及其 95% 置信区间较稳定，通常支持水桥网络具有一定可重复性。",
        data_patterns=("waterbridge_count_combined.csv",),
    ),
    "waterbridge_count_replot": FigureNoteTemplate(
        name="水桥数量重绘图",
        explanation="该图根据已汇总的多重复水桥数量数据重绘三条重复轨迹、跨重复均值及 95% 置信区间，用于与其他综合图版保持统一风格。",
        caption_example="多重复桥连水分子数量重绘图。该图以汇总数据为基础，在统一排版下展示重复趋势、均值曲线和 95% 置信区间。",
        data_patterns=("waterbridge_count_combined.csv",),
    ),
    "rmsd_combined": FigureNoteTemplate(
        name="跨重复 RMSD 汇总图",
        explanation="该图展示多个重复中蛋白主链与配体重原子 RMSD 的三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估构象稳定性的重复性。",
        caption_example="多重复蛋白主链与配体重原子 RMSD 汇总图。重复轨迹、均值曲线及其 95% 置信区间可用于比较结构稳定性在重复之间的一致性。",
    ...
    "min_distance_combined": FigureNoteTemplate(
        name="跨重复最小距离汇总图",
        explanation="该图展示多个重复中配体与蛋白最小重原子距离的三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估结合接触是否稳定。",
        caption_example="多重复配体-蛋白最小距离汇总图。距离曲线若在重复间保持一致，且均值曲线及其 95% 置信区间较稳定，通常支持结合界面接触具有可重复性。",
    ...
    "contact_count_combined": FigureNoteTemplate(
        name="跨重复接触数汇总图",
        explanation="该图展示多个重复中配体-蛋白接触数的三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估界面接触网络的稳定性。",
        caption_example="多重复接触数汇总图。接触数在重复间较一致，且均值曲线及其 95% 置信区间较稳定时，通常支持界面接触网络较稳定。",
        data_patterns=("contact_count_combined.csv",),
    ),
    "hbond_count_combined": FigureNoteTemplate(
        name="跨重复氢键数汇总图",
        explanation="该图展示多个重复中氢键数量的三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估极性结合网络的稳定性。",
        caption_example="多重复氢键数汇总图。氢键数量在重复间较一致，且均值曲线及其 95% 置信区间较稳定时，通常说明极性相互作用模式具有较好可重复性。",
        data_patterns=("hbond_count_combined.csv",),
    ),
    "salt_bridge_count_combined": FigureNoteTemplate(
        name="跨重复盐桥数汇总图",
        explanation="该图展示多个重复中盐桥数量的三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估静电相互作用的持续性。",
        caption_example="多重复盐桥数汇总图。盐桥数量长期保持非零且重复间一致，并且均值曲线及其 95% 置信区间较稳定时，通常支持稳定的静电锚定作用。",
        data_patterns=("salt_bridge_count_combined.csv",),
    ),
    "radius_of_gyration_combined": FigureNoteTemplate(
        name="跨重复回转半径汇总图",
        explanation="该图展示多个重复中蛋白回转半径的三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估整体紧致性的一致性。",
        caption_example="多重复蛋白回转半径汇总图。回转半径在重复间保持接近，且均值曲线及其 95% 置信区间较稳定时，通常说明整体构象紧致性具有较好重复性。",
        data_patterns=("radius_of_gyration_combined.csv",),
    ),
    "buried_surface_combined": FigureNoteTemplate(
        name="跨重复埋藏表面积汇总图",
        explanation="该图展示多个重复中配体结合导致的埋藏表面积三条重复轨迹、跨重复均值以及 95% 置信区间，用于评估界面埋藏稳定性。",
        caption_example="多重复埋藏表面积汇总图。埋藏表面积较大且重复间一致，并且均值曲线及其 95% 置信区间较稳定时，通常支持结合界面较稳定。",
    ...
    "stationary_distribution": FigureNoteTemplate(
        name="MSM 平稳分布图",
        explanation="该图展示所选 MSM 活跃强连通分量中各状态的平稳概率，用于评估长期极限下各状态的相对权重，并结合该分量在全体聚类帧中的覆盖度判断动力学结论的代表性。",
        caption_example="MSM 平稳分布图。平稳概率更高的状态通常代表在所选活跃动力学子空间中更稳定或更常访问的构象状态；若活跃分量覆盖度有限，则应将结果视为探索性结论。",
        data_patterns=("stationary_distribution.csv", "active_state_mapping.csv", "connected_component_summary.csv"),
    ),
    ...
    "state_network": FigureNoteTemplate(
        name="MSM 状态网络图",
        explanation="该图以网络形式展示所选 MSM 活跃分量中状态间的平衡转移通量和各状态的相对权重，并在可能时标注平均首达时间，用于直观理解主要动力学交换结构。",
        caption_example="MSM 状态网络图。节点大小反映平稳概率，边的粗细反映平衡转移通量 pi_i P_ij，边标签给出对应转移概率及平均首达时间信息。",
        data_patterns=("transition_matrix.csv", "stationary_distribution.csv", "equilibrium_transition_flux.csv", "mean_first_passage_times.csv", "active_state_mapping.csv"),
    ),
    ...
    "chapman_kolmogorov_test": FigureNoteTemplate(
        ...
        explanation="该图比较基准 lag 下 MSM 对更长时间尺度转移概率的预测与在对应更长 lag 下独立估计得到的结果，是比 ITS 更强的 MSM 合理性检验。",
        caption_example="Chapman-Kolmogorov 验证图。模型预测曲线与独立估计点越接近，通常越支持该 MSM 在对应时间尺度上具有较好的马尔可夫近似。",
        data_patterns=("chapman_kolmogorov_test.csv", "msm_notes.json", "active_state_mapping.csv"),
    ),

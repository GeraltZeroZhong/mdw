...
def _export_cluster_snapshots(replica_dirs, trajectories, labels, embed, frame_owner, n_clusters_use: int, centers, out_dir: Path, cfg: AdvancedAnalysisConfig):
    ...
    snapshot_entries = []
    rep_rows = []
    top_clusters = sorted(range(n_clusters_use), key=lambda c: int(np.sum(labels == c)), reverse=True)[: max(1, min(cfg.representative_snapshot_clusters, n_clusters_use))]
    for cluster_id in range(n_clusters_use):
        ...
        traj[int(frame_idx)].save_pdb(str(pdb_path))
        rep_rows.append([cluster_id, replica_name, int(frame_idx), float(np.sqrt(np.min(d2))), str(pdb_path)])
        if cluster_id in top_clusters:
            protein_ca = traj.topology.select("protein and name CA")
            ligand_res = next(res for res in traj.topology.residues if (not res.is_protein) and res.name.upper() not in {"HOH", "WAT", "SOL", "NA", "CL", "K", "MG", "CA", "ZN", "MN", "FE", "CU", "CS", "RB", "LI", "F", "BR", "I"})
            ...
            snapshot_entries.append({
                ...
                "protein_xyz": traj.xyz[int(frame_idx), protein_ca, :],
                "ligand_xyz": traj.xyz[int(frame_idx), ligand_heavy, :],
            })
    save_csv(out_dir / "representative_frames.csv", ["cluster", "replica", "frame", "distance_to_center", "pdb_path"], rep_rows)
...
def run_advanced_analysis(
    ...
    if plot_snapshots:
        ...
        snapshot_entries = _export_cluster_snapshots(replica_dirs, trajectories, labels, embed, frame_owner, n_clusters_use, kmeans.cluster_centers_, Path(cfg.analysis_root) / "snapshots", cfg)
        plot_snapshot_grid(snapshot_entries, Path(cfg.analysis_root) / "snapshots" / "representative_state_snapshots", style, title="Representative structural states")

...
from md_workbench.workflows import prepare_next_replica_workflow_config, run_next_replica_workflow
...
if __name__ == "__main__":
    ...
    cfg = prepare_next_replica_workflow_config(load_workflow_config(config_path))
    log_session = start_run_log(cfg.workspace_root, "next_replica_workflow")
    log_session.log(f"Config path: {Path(config_path).resolve()}")
    log_session.log_json("Workflow configuration", cfg.to_dict())
    try:
        outputs = run_next_replica_workflow(cfg)
